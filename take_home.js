...
// 1. Buat array baru berdasarkan script pada link diatas
...
const transaksiByLokasi = () => {
    const transaksi = []
    data.forEach(d => {
        storeLocation = d.storeLocation,
        detail = {...d}
        const lokasi = transaksi.findIndex(value => value.storeLocation == storeLocation)
        if(lokasi == -1){
            transaksi.push({storeLocation, detail: [detail], })
        } else {
            transaksi[lokasi].detail.push(detail)
        }
        
    })
    return transaksi
}
console.log(transaksiByLokasi(), '\n\n')
console.log('=====================================================');


// 3. Buat array berdasarkan script pada link diatas yang menyimpan keuntungan yang didapat dari setiap transaksi
const keuntunganTransaksi = data.map(data => {
    let keuntungan = 0
    data.items.forEach(value => {
        keuntungan += value.price.$numberDecimal * value.quantity.$numberInt
    })
    return {
...
const ratingToko = () => {
    ...
    // Membuat object baru sekaligus mendapatkan store location tanpa duplikat
    data.forEach(e => {
        const { storeLocation } = e
        let checkToko = arrToko.find(value => value.storeLocation == storeLocation)
        if(!checkToko){
            arrToko.push({storeLocation, satisfaction: [], rataRata: 0})
        }
    })

    // Menambahkan satisfaction pada objek sebelumnya
    arrToko.forEach(e => {
        data.forEach(d => {
            let satisfaction = d.customer.satisfaction.$numberInt
            const { storeLocation } = d
            if(e.storeLocation == storeLocation){
                satisfaction = parseInt(satisfaction)
                e.satisfaction.push(satisfaction)
            }
        })

        // Menambahkan rata rata pada objek sebelumnya
        let jumlahRating = 0
        for(r of e.satisfaction){
            jumlahRating += r
        }
        let rataRata = (jumlahRating / e.satisfaction.length).toFixed(2)
        e.rataRata = parseFloat(rataRata)
    })

    return arrToko
}
console.log(ratingToko(), '\n\n')
console.log('=====================================================');
...
const keuntunganToko = () => {
    // Membuat array kosong dan memasukan lokasi tidak duplikat
    let keuntungan = [];
    data.forEach(value => {
        const { storeLocation } = value
        const checkToko = keuntungan.find(e => e.storeLocation == storeLocation)
        checkToko || keuntungan.push({storeLocation, keuntungan: 0})
    })

    keuntungan.forEach(value => {
        data.forEach(d => {
            let jumlahKeuntungan = 0
            // Menghitung keuntungan setiap transaksi
            d.items.forEach(item => {
                const price = parseFloat(item.price.$numberDecimal)
                const quantity = parseInt(item.quantity.$numberInt)
                jumlahKeuntungan += price * quantity
            })
            // Menambahkan setiap jumlah transaksi ke store location
            if(d.storeLocation == value.storeLocation){
                value.keuntungan += jumlahKeuntungan
            }
        })
    })
